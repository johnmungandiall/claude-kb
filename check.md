...
3. RUN `bash tools/kb-check.sh --fix` — it auto-corrects every drifted `:line`
   whose named symbol it can locate unambiguously, and prints what it changed.
   Then RUN `bash tools/kb-check.sh` (plain) and resolve whatever is left: a
   `BROKEN` line is an explicit `path:line` whose file is gone or whose line is
   past EOF — fix the path / line by hand; a `STALE` line is name-anchored and the
   symbol couldn't be auto-located (overloaded or a getter) — grep the name and set
   the right line. Re-run until it prints `broken 0`. Do NOT report done until it does.
...
# RULES
- FOCUSED: the only edits you make are creating the two files, running `--fix`, and
  resolving the leftover flags — no KB rebuild, no note rewrites, no new features.
- Write the script byte-for-byte as given above; don't change its behaviour.
- Trust `--fix` only for the line numbers it reports changing; eyeball anything you
  edit by hand. Never copy the hook into `.git/hooks/` without the user's say-so.

# OUTPUT
Say whether `tools/kb-check.sh` ends in `broken 0`, and list what you created, the
`:line`s `--fix` corrected, and any pointers you fixed by hand.
