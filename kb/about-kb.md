...
## This repo (claude-kb specifics)
- The prompts DUPLICATE shared blocks (lockstep sets) — before editing any prompt,
  read [[edit-a-prompt]]; gates: `bash tools/kb-check.sh` AND `bash tools/lockstep-check.sh`
  both green before done. Traps in [[gotchas]].
