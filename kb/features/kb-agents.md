...
- **Auto-created:** `prompt.md` STEP 6 (`prompt.md:142-160`) and `update.md` step 7 (`update.md:123-132`) write these into any target repo, so every claude-kb setup gets them automatically.
...
- `kb-maintainer` stays DRY — reads `kb/about-kb.md` (or the `## Knowledge Base` rules) instead of re-embedding them. `kb-verify` / `kb-slim` carry a DISTILLED copy of their prompt — NOT byte-synced, since agents are generated artifacts, not README-mirrored copies.
