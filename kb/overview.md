...
- **Entry points:** `prompt.md` (first-time setup), `update.md` (upgrade existing KB), `verify.md` (audit KB vs code), `slim.md` (shrink a bloated `CLAUDE.md`), `.claude/agents/` (KB subagents) + `tools/kb-check.sh` (drift checker), both auto-created by the prompts, `README.md` (human docs + copy-paste blocks).
...
- **last indexed:** 2026-06-27 / v2.8 — full version history in [[changelog]] (single source of truth; not duplicated here).
