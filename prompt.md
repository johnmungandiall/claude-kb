...
3. Write the KB as a TREE of tiny files:
   ...
   - `kb/runbooks/<name>.md`— one per multi-step procedure (release, deploy, secret rotation, onboarding): EVERY file/step/artifact to touch, in order — not just build commands (only if any exist)
...
```
   ...
   Then run `bash tools/kb-check.sh --fix` (auto-repairs any drifted line numbers)
   followed by `bash tools/kb-check.sh` to confirm `broken 0`, and tell the user how
...
- NO SILENT DRIFT: capture cross-cutting invariants while you build. A "lockstep set"
  (a value/name/contract repeated in >1 place: version strings, a mirrored enum, a
  duplicated allowlist, a server field parsed client-side) gets a "change X → also Y, Z"
  note cross-linked to each member; every feature-note trap is also stubbed into the
  central `kb/gotchas.md`; every multi-step procedure gets a `kb/runbooks/<name>.md`.
...
# OUTPUT
Create/update the files under `kb/`, `.claude/agents/`, and `tools/`, and update the
`## Knowledge Base` section in `CLAUDE.md`. Before finishing, RUN
`bash tools/kb-check.sh --fix` then `bash tools/kb-check.sh` — this auto-repairs
drifted line numbers, confirms the KB's pointers resolve, AND proves you actually
created the tool. If it fails because the file is missing, you skipped STEP 7: create
`tools/kb-check.sh` and `tools/hooks/pre-commit` now, then re-run it. Do NOT report
done until it ends in `broken 0`. Then print a short list of what changed.
